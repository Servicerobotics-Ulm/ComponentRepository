...
#include "ComponentQtRobotConsoleOpcUaBackendPortFactory.hh"
...
static ComponentQtRobotConsoleOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentQtRobotConsoleOpcUaBackendPortFactory::ComponentQtRobotConsoleOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentQtRobotConsole::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentQtRobotConsoleOpcUaBackendPortFactory::~ComponentQtRobotConsoleOpcUaBackendPortFactory()
{  }

void ComponentQtRobotConsoleOpcUaBackendPortFactory::initialize(ComponentQtRobotConsole *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentQtRobotConsoleOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentQtRobotConsoleOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
int ComponentQtRobotConsoleOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentQtRobotConsoleOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentQtRobotConsoleOpcUaBackendPortFactory::destroy()
{
