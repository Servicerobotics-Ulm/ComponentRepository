...
void EnvironmentQueryServiceAnswHandler::handleQuery(const Smart::QueryIdPtr &id, const CommObjectRecognitionObjects::CommObjectRecognitionId& request) 
{
	...
	std::cout << ":EnvironmentQueryServiceAnswHandler : query request for environment with id = " << request.get_id()<< std::endl;
	std::cout << ":EnvironmentQueryServiceAnswHandler : currently detected environment id     = " << COMP->environmentIdCounter<< std::endl;
