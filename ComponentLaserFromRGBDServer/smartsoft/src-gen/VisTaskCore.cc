...
VisTaskCore::VisTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
void VisTaskCore::notify_all_interaction_observers() {
	...
	if(const VisTask* visTask = dynamic_cast<const VisTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(visTask);
		}
	}
}

void VisTaskCore::attach_interaction_observer(VisTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void VisTaskCore::detach_interaction_observer(VisTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int VisTaskCore::execute_protected_region()
{
...
void VisTaskCore::updateAllCommObjects()
{
...
void VisTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int VisTaskCore::getPreviousCommObjId()
{
