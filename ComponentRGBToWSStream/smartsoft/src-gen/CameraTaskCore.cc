...
CameraTaskCore::CameraTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
CameraTaskCore::~CameraTaskCore()
{
}

int CameraTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int CameraTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int CameraTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void CameraTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const CameraTask* cameraTask = dynamic_cast<const CameraTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(cameraTask);
		}
	}
}

void CameraTaskCore::attach_interaction_observer(CameraTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void CameraTaskCore::detach_interaction_observer(CameraTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int CameraTaskCore::execute_protected_region()
{
...
void CameraTaskCore::updateAllCommObjects()
{
...
void CameraTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int CameraTaskCore::getPreviousCommObjId()
{
