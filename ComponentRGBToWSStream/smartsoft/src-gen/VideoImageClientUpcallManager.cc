...
void VideoImageClientUpcallManager::notify_upcalls(const DomainVision::CommVideoImage &input)
{
	for(auto it=upcalls.begin(); it!=upcalls.end(); it++) {
		(*it)->on_VideoImageClient(input);
	}
}

void VideoImageClientUpcallManager::attach(VideoImageClientUpcallInterface *upcall)
{
	upcalls.push_back(upcall);
}
void VideoImageClientUpcallManager::detach(VideoImageClientUpcallInterface *upcall)
{
