...
/**
 * This file implements the classes GFSPair, GFSPTreeNode, GFSPTree, GTreeDiamAlg,
 * g_heap_pairs_p, ProjPointSet, point2d, MinAreaRectangle ... and lots of global functions.
 */
...
const int HEAP_LIMIT = 10000;
const int HEAP_DELTA = 10000;
const double MVBB_EPS = 0.01;
const double PI = 3.14159265958979323846;
...
private:
  ...
  gdiam_point* p_pnt_left;
  gdiam_point* p_pnt_right;
  
  gdiam_real bbox_diam;
  gdiam_real bbox_diam_proj;
...
public:
  ...
  friend class  GFSPTree; //WHY HERE???
...
public:
  ...
  {
    arr = new gdiam_point[sizeof( gdiam_point ) * size];
    assert( arr != NULL );
  ...
  // void findExtremeInProjection( GFSPPair& pair, GPointPair& max_pair_diam ); //TODO: implemented anywhere?
};
...
{	
  ...
  for  ( int ind = 0; ind < 8; ind++ ) 
  {
    ...
    for ( int jnd = 0; jnd < 8; jnd++ ) 
    {
      ...
      rl = std::max( rl, pnt_distance( a, b, proj ) );
    }
  }
...
public:
  ...
  void init( GFSPTreeNode* _left, GFSPTreeNode* _right,	gdiam_point proj_dir, gdiam_real dist ) 
  {
    ...
    max_diam = std::max( std::max( bbox_proj_dist( _left->getBBox(), _right->getBBox(), proj_dir ),
  ...
  {			
    ...
    two_r = std::max( left->maxDiam(), right->maxDiam() );
...
void heap_init( heap_t* pHeap, ptrCompareFunc _pCompFunc ) 
{
  ...
  pHeap->max_size = 100;
  pHeap->pArr = new voidPtr_t [ sizeof( void * ) * pHeap->max_size ];
...
static void resize( heap_t* pHeap, int  size ) 
{
  ...
  max_sz = size * 2;
  pTmp = new voidPtr_t [ max_sz * sizeof( void* ) ];
  assert( pTmp != NULL );
...
int compare_pairs( void* _a, void* _b ) 
{
  ...
  if  ( a.maxDiam() < b.maxDiam() ) return -1;
  if  ( a.maxDiam() > b.maxDiam() ) return 1;
...
void computeExtremePair( const gdiam_point* arr, int size, int dim, GPointPair& pp ) 
{
  ...
  for ( int ind = 1; ind < size; ind++ ) 
  {
    ...
    if ( pnt[ dim ] < pp.p[ dim ] ) pp.p = pnt;
    if ( pnt[ dim ] > pp.q[ dim ] ) pp.q = pnt;
  }
...
////////////////////////////////////////////////////////
/////// implementations for class GTreeDiamAlg ////////
//////////////////////////////////////////////////////
...
void  GTreeDiamAlg::addPairHeap( g_heap_pairs_p& heap, GFSPTreeNode* left, GFSPTreeNode* right ) 
{
...
void GTreeDiamAlg::addPairHeap( g_heap_pairs_p& heap, GFSPTreeNode* left, GFSPTreeNode* right, gdiam_point proj, GFSPPair& father ) 
{
  const gdiam_point p( *(left->ptr_pnt_left()) );
  const gdiam_point q( *(right->ptr_pnt_left()) );
...
void  GTreeDiamAlg::split_pair_proj( GFSPPair& pair, g_heap_pairs_p& heap, double eps, gdiam_point proj ) 
{
  ...
  if ( f_is_left_splitable && f_is_right_splitable ) {
    ...
    if ( pair.get_left() != pair.get_right() )
      addPairHeap( heap, pair.get_left()->get_right(), pair.get_right()->get_left(), proj, pair );
...
void  GTreeDiamAlg::split_pair( GFSPPair& pair, g_heap_pairs_p& heap, double eps ) 
{
  bool f_is_left_splitable, f_is_right_splitable;
...
{
  gdiam_point* p_arr = gdiam_convert( start, size );
  GPointPair pair = gdiam_approx_diam( p_arr, size, eps );
  //free( p_arr );
  delete(p_arr);  
  return  pair;
}
...
{	
  ...
  gdiam_point* p_arr = new gdiam_point[sizeof( gdiam_point ) * size];
  assert( p_arr != NULL );
...
public:
  ...
  bool equal( const point2d  pnt ) const 
  {
    ...
    VALGRIND_CHECK_DEFINED(y);
    VALGRIND_CHECK_DEFINED(pnt.x);
    VALGRIND_CHECK_DEFINED(pnt.y);
    */
...
{
  // here was > 0 before (resulting in problems)
...
point2d_ptr get_min_point( vec_point_2d& in, int& index ) 
{
  ...
  for ( int ind = 0; ind < (int)in.size(); ind++ ) 
  {
    ...
    {
      if ( ( in[ ind ]->y == min_pnt->y ) && ( in[ ind ]->x < min_pnt->x ) ) {
	min_pnt = in[ ind ];
	index = ind;
      }
    }
...
public:
  ...
  void compute_edge_dir( unsigned int u ) 
  {
    if (ch.size() == 0)
      return;
  ...
  /* Computing the intersection point of two lines, each given by a	point and slope */
		    ...
		    gdiam_real  & area ) 
  {
    ...
    area = pnt_distance_2d( bbox.vertices[ 0 ],
			    bbox.vertices[ 1 ] ) * pnt_distance_2d( bbox.vertices[ 0 ],	bbox.vertices[ 3 ] );
  };
  ...
  void compute_min_bbox_inner( bbox_2d_info& min_bbox, gdiam_real  & min_area ) 
  {
    ...
    angles = new gdiam_real [ sizeof( gdiam_real ) * (int)ch.size() ];
    memset(angles, 0, sizeof( gdiam_real ) * (int)ch.size() * sizeof(gdiam_real));
    assert( angles != NULL );
...
static void construct_base_inner( gdiam_point  pnt1, gdiam_point  pnt2, gdiam_point  pnt3 ) {
	
	pnt_normalize( pnt1 );
	pnt_normalize( pnt2 );
	pnt_normalize( pnt3 );

	if ( ( pnt_length( pnt1 ) < ZERO_EPS ) && ( pnt_length( pnt2 ) < ZERO_EPS ) && ( pnt_length( pnt3 ) < ZERO_EPS ) ) {
		assert( false );
	}
...
public:
  ...
  {		
    ...
    if ( pnt_length( dir ) == 0.0 ) 
    {
      ...
      //fflush( stdout );
      //fflush( stderr );
      //JS changed from assert to throw
    ...
    arr = new point2d[size * sizeof( point2d )];
    assert( arr != 0 );
...
gdiam_bbox gdiam_mvbb_optimize( gdiam_point* start, int size, gdiam_bbox bb_out, int times ) 
{
  ...
  //printf( "\ngdiam_mvbb_optimize called\n" );fflush( stdout );

  for ( int ind = 0; ind < times; ind++ ) 
  {
    //printf("pps.init "); fflush( stdout );
    ProjPointSet pps;    
    ...
    //printf( "Pre Init - " );  fflush( stdout );

    pps.init( bb_out.get_dir( ind % 3 ), start, size );    
    //printf( "Compute\n" ); fflush( stdout );
    pps.compute();
        
    //printf( "Old volume: %g ", bb_out.volume() );
    bb_tmp = pps.get_bbox();
    //printf( "New volume: %g\n", bb_tmp.volume() );
    // 	printf( "Old bounding box:\n" );
    // 	bb2.dump();

    if (  bb_tmp.volume() > 0 && bb_tmp.volume() < bb_out.volume() )
      bb_out = bb_tmp;
  }

  //printf( "gdiam_mvbb_optimize end\n" );
  return  bb_out;
}
...
static int gcd3( int a, int b, int c ) 
{
  ...
  b = abs( b );
  c = abs( c );
  if ( a == 0 ) return gcd2( b, c );
  if ( b == 0 ) return gcd2( a, c );
  if ( c == 0 ) return gcd2( a, b );
  
  return gcd2( a, gcd2( b, c ) );
}


static void try_direction( gdiam_bbox& bb, gdiam_bbox& bb_out, gdiam_point* start, int size, int x_coef, int y_coef, int z_coef ) 
{
  ...
  //printf( "newdir: (%f, %f, %f) trying: (%d, %d, %d) ", new_dir[0], new_dir[1], new_dir[2], x_coef, y_coef, z_coef );
  if (fabs(new_dir[0])+fabs(new_dir[1])+fabs(new_dir[2]) == 
      fabs(x_coef)+fabs(y_coef)+fabs(z_coef))
  {
    //cout << " - fail\n";
    return;
  }
  ...
  if ( bb_new.volume() > 0)
  {
    ...
    if ( bb_new.volume() > 0 && bb_new.volume() < bb_out.volume() )
      bb_out = bb_new;
  }
}
...
{       	
  ...
  bb_out = bb = gdiam_approx_const_mvbb( start, size, MVBB_EPS, NULL );
  //cout << "volume A: " << bb.volume() << endl;
  ...
  //cout << "volume C: " << bb_out.volume() << endl;
...
static void register_point( gdiam_point pnt, gdiam_point* tops, gdiam_point* bottoms, int grid_size, gdiam_bbox& bb ) 
{
  ...
  // 	assert( ( -1 <= x_ind ) && ( x_ind <= grid_size ) );//TODO: This was violated once, x_ind was -2147483648 ?!?!?!?!?!
  if ( !( ( -1 <= x_ind ) && ( x_ind <= grid_size ) ) ) 
  {
    throw (47);
    return;
  }
  ...
  if ( x_ind < 0 ) x_ind = 0;
  if ( x_ind >= grid_size ) x_ind = grid_size - 1;

  // 	y_ind
  y_ind = (int)( pnt_bb[ 1 ] * grid_size );
	
  // 	assert( ( -1 <= y_ind )  &&  ( y_ind <= grid_size ) );
  if ( !( ( -1 <= y_ind ) && ( y_ind <= grid_size ) ) ) 
  {
    throw (47);
    return;
  }
  ...
  if ( y_ind < 0 ) y_ind = 0;
  if ( y_ind >= grid_size ) y_ind = grid_size - 1;
  ...
  if ( pnt_top[ 2 ] < pnt_bb[ 2 ] ) tops[ position ] = pnt;

  bb.get_normalized_coordinates( bottoms[ position ], pnt_bottom );
  if ( pnt_bottom[ 2 ] > pnt_bb[ 2 ] ) bottoms[ position ] = pnt;
}
...
// the point-set. The only guarantee is that if we use sample of size m,
...
{	
  ...
  grid_size = (int)sqrtl( sample_size / 2 );
  
  grid_entries = grid_size * grid_size;
  mem_size = (int)( sizeof( gdiam_point ) * grid_size * grid_size );
  
  bottoms = new gdiam_point[ mem_size ];
  tops = new gdiam_point [ mem_size ];
  out_arr = new gdiam_point [ sizeof( gdiam_point ) * sample_size ];
  ...
  {
    out_arr[ out_count++ ] = start[ rand() % size ]; //start[0];//
    //printf("ADD sample: %f, %f, %f\n", out_arr[out_count-1][0], out_arr[out_count-1][1], out_arr[out_count-1][2]);
  }
  delete[] tops;
  delete[] bottoms;
...
gdiam_bbox gdiam_approx_mvbb_grid_sample( gdiam_point* start, int size, int grid_size, int sample_size ) {
	...
	gdiam_point* sample = NULL;

// 	cout << "gdiam_approx_mvbb_grid_sample(...) " << sample_size << "/" << size << std::endl;
	if  ( sample_size >= size )
	  return gdiam_approx_mvbb_grid( start, size, grid_size );
        
//	std::cout << "gdiam_approx_mvbb_grid_sample(...) called" << std::endl;
// 	fflush( stdout );

	bb = gdiam_approx_const_mvbb( start, size, MVBB_EPS, NULL );
//	printf( "volume on sample: %g\n", bb.volume() );
// 	printf( "guess from const approximation:\n" );

	sample = gdiam_convex_sample( start, size, bb, sample_size );
// 	printf("convex sample established\n");
