...
void NavigationVelocityHandler::on_navigationVelocityServer(const CommBasicObjects::CommNavigationVelocity &input)
{
	if (COMP->getGlobalState().getRobot().getVerbose()){
		std::cout << "[NavigationVelocityHandler] commanded v, omega: (" << input.get_vX(1) << ", " << input.get_omega() << ")\n";
	}

	// TODO: Check if correct
	COMP->robot.setV(input.get_vX(1));
	COMP->robot.setOmega(input.get_omega());
}
