...
#define RMP_CAN_CMD_ENABLE_MODE		15
#define RMP_CAN_CMD_SET_MODE		16
...
#define RMP_COUNT_PER_M			40181
#define RMP_COUNT_PER_DEG		7.8
//#define RMP_COUNT_PER_M_PER_S		332
#define RMP_COUNT_PER_M_PER_S		401
...
#define RMP_COUNT_PER_REV               117031

#define RMP_MAX_TRANS_VEL_MM_S		1666
...
#define RMP_COUNT_PER_NM		1463

//#define RMP_GEOM_WHEEL_SEP 0.54
#define RMP_GEOM_WHEEL_SEP 0.41
...
public:
	...
	// Is this frame ready (i.e., did we get all 8 messages)?
	bool IsReady() {
		return ready == 0xFF;
	}
 ...
 * returns:
 */
inline void rmp_frame_t::AddPacket(const CanPacket &pkt) {
	bool known = true;
...
//		std::cout << left_dot << std::endl;
//		bin(pkt.msg[0] , std::cout);
//		std::cout << "  ";
//		bin(pkt.msg[1] , std::cout);
//		std::cout << std::endl << std::endl;
	...
	case RMP_CAN_ID_MSG4:
		left = (uint32_t) (((uint32_t) pkt.GetSlot(1) << 16) | (uint32_t) pkt.GetSlot(0));
		right = (uint32_t) (((uint32_t) pkt.GetSlot(3) << 16) | (uint32_t) pkt.GetSlot(2));
		break;

	case RMP_CAN_ID_MSG5:
		foreaft = (uint32_t) (((uint32_t) pkt.GetSlot(1) << 16) | (uint32_t) pkt.GetSlot(0));
		yaw = (uint32_t) (((uint32_t) pkt.GetSlot(3) << 16) | (uint32_t) pkt.GetSlot(2));
		break;
	...
	case RMP_CAN_ID_MSG7:
		mode = (int16_t) pkt.GetSlot(0);
		gain = (int16_t) pkt.GetSlot(1);
		ui_battery = (int16_t) pkt.GetSlot(2);
		power_battery = (int16_t) pkt.GetSlot(3);
		break;

	case RMP_CAN_ID_MSG8:
		velocity_cmd = (int16_t) pkt.GetSlot(0);
		turn_cmd = (int16_t) pkt.GetSlot(1);
		break;
