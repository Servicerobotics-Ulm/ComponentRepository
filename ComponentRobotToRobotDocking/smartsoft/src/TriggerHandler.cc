...
void TriggerHandler::handleCommRobotinoObjects_RobotinoDockingParameter_SAVE_BEFORE_DOCKING_POSE()
{
	...
	std::cout<<"[TriggerHandler] Save current Position!"<<std::endl;
		COMP->baseStatePushClient->getUpdateWait(COMP->savedBaseState);
		COMP->savedBaseState_valid = true;
		//std::cout<<"[TriggerHandler] Saved current Pose: "<<COMP->savedBaseState.getBaseOdomPose().getPose3D()<<std::endl;
		std::cout<<  "[TriggerHandler]Saved current Pose: "
				 << COMP->savedBaseState.getBaseOdomPose().get_x(1) << " "
				 << COMP->savedBaseState.getBaseOdomPose().get_y(1) << " "
				 << COMP->savedBaseState.getBaseOdomPose().get_base_azimuth()*(180.0f/3.1415) << "deg" << std::endl;
}
