...
PoseUpdateTaskCore::PoseUpdateTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
PoseUpdateTaskCore::~PoseUpdateTaskCore()
{
}

int PoseUpdateTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int PoseUpdateTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int PoseUpdateTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
