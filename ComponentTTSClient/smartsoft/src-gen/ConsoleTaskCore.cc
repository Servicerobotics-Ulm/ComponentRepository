...
int ConsoleTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int ConsoleTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int ConsoleTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
...
void ConsoleTaskCore::notify_all_interaction_observers() {
	...
	if(const ConsoleTask* consoleTask = dynamic_cast<const ConsoleTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(consoleTask);
		}
	}
}

void ConsoleTaskCore::attach_interaction_observer(ConsoleTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void ConsoleTaskCore::detach_interaction_observer(ConsoleTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
