...
static ComponentKBAcePortFactory acePortFactory;

ComponentKBAcePortFactory::ComponentKBAcePortFactory()
{  
	componentImpl = 0;
	ComponentKB::instance()->addPortFactory("ACE_SmartSoft", this);
}

ComponentKBAcePortFactory::~ComponentKBAcePortFactory()
{  }

void ComponentKBAcePortFactory::initialize(ComponentKB *component, int argc, char* argv[])
{
	if(component->connections.component.defaultScheduler != "DEFAULT") {
		...
		componentImpl = new ComponentKBImpl(component->connections.component.name, argc, argv, sched_params);
	} else {
		// create new instance of the SmartSoft component
		componentImpl = new ComponentKBImpl(component->connections.component.name, argc, argv);
	}
}

int ComponentKBAcePortFactory::onStartup()
{
...
SmartACE::SmartComponent* ComponentKBAcePortFactory::getComponentImpl()
{
	return componentImpl;
}

int ComponentKBAcePortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
	componentImpl->stopComponentInfrastructure(timeoutTime);
	return 0;
}

void ComponentKBAcePortFactory::destroy()
{
