...
void TriggerHandler::handleCommLocalizationObjects_LocalizationParameter_SENSORSTOUSE(const unsigned short &sensorsToUse)
{
	SmartACE::SmartRecursiveGuard pfGuard(COMP->PFMutex);
	{
		...
		{
		case 0:
			COMP->sensor_to_use = amcl_sensor_to_use::LASER_ONLY;
			std::cout << "[TriggerHandler] Only Laser will be used for Localization"<<std::endl;
			break;
		case 1:
			COMP->sensor_to_use = amcl_sensor_to_use::LASER_PLUS_VISUAL_TAGS;
			std::cout << "[TriggerHandler] Both Laser and VisualTags will be used for Localization"<<std::endl;
			break;
		case 2:
			COMP->sensor_to_use = amcl_sensor_to_use::VISUAL_TAGS_ONLY;
			std::cout << "[TriggerHandler] Only VisualTags will be used for Localization"<<std::endl;
			break;
		default:
			std::cout << "[TriggerHandler] Unknown param " << sensorsToUse <<" use only 0,1,2 for this trigger"<<std::endl;
			break;
		}
