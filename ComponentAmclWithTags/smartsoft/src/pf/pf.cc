...
// Initialize the filter using multiple gaussians
void pf_init_multiple_gaussians(pf_t *pf, pf_vector_t mean, pf_matrix_t cov)
{
  int i;
  pf_sample_set_t *set;
  pf_sample_t *sample;
  ...
  while(set_b->sample_count < pf->max_samples)
  {
    ...
    else
    {
      ...
      */
      ...
      for(i=0;i<set_a->sample_count;i++)
      {
    	  //printf("i = %d, r = %f, ci = %f,  ci1 = %f\n", i, r, c[i],c[i+1]);
    	  if((c[i] <= r) && (r < c[i+1]))
          break;
      }
      //printf("i = %d, a sample_count = %d", i, set_a->sample_count);
