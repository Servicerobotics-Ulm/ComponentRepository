...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  total_weight = 0.0;
  //std::cout << "[AMCLLaser] sample_count : " << set->sample_count <<std::endl;
  // Compute the sample weights
  for (j = 0; j < set->sample_count; j++)
  {
    sample = set->samples + j;
    ...
    sample->weight *= p;
    //std::cout << "laser particle i=" <<j << " weight: " <<  sample->weight << " new mul : " << p <<std::endl;
    total_weight += sample->weight;
  }
  //std::cout << " #######################################   laser total_weight =" <<total_weight <<std::endl;
  return(total_weight);
}
