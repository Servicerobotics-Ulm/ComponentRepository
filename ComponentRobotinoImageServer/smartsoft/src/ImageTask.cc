...
int ImageTask::on_execute()
{
	...
	if(COMP->stateSlave->acquire("pushimage") == Smart::SMART_OK)
	{
			...
			// get newest image from RobotinoAPITask
			if(localState.getSettings().getDebug_info()){
				std::cout<<__FUNCTION__<<" before getImage"<<std::endl;
			}
			*image = COMP->robotinoAPITask->getImage();
			if(localState.getSettings().getDebug_info()){
				std::cout<<__FUNCTION__<<" past getImage"<<std::endl;
			}
			...
			if (localState.getBase().getOn_ptu()) {
				...
				Smart::StatusCode status = COMP->devicePoseUpdateServiceIn->getUpdate(devicePoseState);
			...
			//cvSaveImage("SmartRobotinoImageServer-image.jpg",TESTIMAGE);
			//cvReleaseImage(&TESTIMAGE);


			COMP->rGBImagePushServiceOut->put(*image);
			if (localState.getPush_newest().getDebug_info()) {
				std::cout << "Newest: Image sent. Sequ: "<<image->getSeq_count()<<std::endl;
			}
...
//	DomainVision::CommVideoImage *img = 0;
//	while (true) {
//		_queue_sem.acquire();
...
//}
//
//void ImageTask::ImageCleanupThread::_enqueue(DomainVision::CommVideoImage *image) {
//	_queue_mutex.acquire();
//	_queue.push(image);
//	_queue_mutex.release();
//	_queue_sem.release();
