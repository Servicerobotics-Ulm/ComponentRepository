...
PackingTaskCore::PackingTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
PackingTaskCore::~PackingTaskCore()
{
}

int PackingTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int PackingTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int PackingTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void PackingTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const PackingTask* packingTask = dynamic_cast<const PackingTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(packingTask);
		}
	}
}

void PackingTaskCore::attach_interaction_observer(PackingTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void PackingTaskCore::detach_interaction_observer(PackingTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int PackingTaskCore::execute_protected_region()
{
...
void PackingTaskCore::updateAllCommObjects()
{
...
Smart::StatusCode PackingTaskCore::objectEventServerPut(CommObjectRecognitionObjects::CommObjectRecognitionEventState &eventState)
{
...
void PackingTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int PackingTaskCore::getPreviousCommObjId()
{
