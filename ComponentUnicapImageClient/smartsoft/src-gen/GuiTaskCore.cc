...
GuiTaskCore::GuiTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
GuiTaskCore::~GuiTaskCore()
{
}


void GuiTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const GuiTask* guiTask = dynamic_cast<const GuiTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(guiTask);
		}
	}
}

void GuiTaskCore::attach_interaction_observer(GuiTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void GuiTaskCore::detach_interaction_observer(GuiTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int GuiTaskCore::execute_protected_region()
{
...
void GuiTaskCore::updateAllCommObjects()
{
...
void GuiTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int GuiTaskCore::getPreviousCommObjId()
{
