...
void CompHandler::onStartup() 
{
	...
	//	status = COMP->connectAndStartAllServices();
		...
		if(COMP->getGlobalState().getSettings().getUseNewest() == true){
			status = COMP->connectPushNewestClient(COMP->connections.pushNewestClient.serverName, COMP->connections.pushNewestClient.serviceName);
		}

		if(COMP->getGlobalState().getSettings().getUseQuery() == true){
			status = COMP->connectQueryClient(COMP->connections.queryClient.serverName, COMP->connections.queryClient.serviceName);
		}
