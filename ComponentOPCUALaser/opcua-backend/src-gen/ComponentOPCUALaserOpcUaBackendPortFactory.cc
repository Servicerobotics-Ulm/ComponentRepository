...
#include "ComponentOPCUALaserOpcUaBackendPortFactory.hh"
...
static ComponentOPCUALaserOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentOPCUALaserOpcUaBackendPortFactory::ComponentOPCUALaserOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentOPCUALaser::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentOPCUALaserOpcUaBackendPortFactory::~ComponentOPCUALaserOpcUaBackendPortFactory()
{  }

void ComponentOPCUALaserOpcUaBackendPortFactory::initialize(ComponentOPCUALaser *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentOPCUALaserOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentOPCUALaserOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<CommBasicObjects::CommBaseState> * ComponentOPCUALaserOpcUaBackendPortFactory::createBaseStateServiceIn()
{
	return new SeRoNet::OPCUA::Client::PushClient<CommBasicObjects::CommBaseState>(componentImpl);
}


Smart::IQueryServerPattern<CommBasicObjects::CommVoid, CommBasicObjects::CommMobileLaserScan> * ComponentOPCUALaserOpcUaBackendPortFactory::createLaserQueryServiceAnsw(const std::string &serviceName)
{
...
Smart::IPushServerPattern<CommBasicObjects::CommMobileLaserScan> * ComponentOPCUALaserOpcUaBackendPortFactory::createLaserServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<CommBasicObjects::CommMobileLaserScan>(componentImpl, serviceName);
}


int ComponentOPCUALaserOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentOPCUALaserOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentOPCUALaserOpcUaBackendPortFactory::destroy()
{
