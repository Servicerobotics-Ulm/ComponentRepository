...
//  printf("setScanDataCfg: TX: %s", buf);
  com.send(buf, strlen(buf));
...
std::string LMS1xx::statusToString(status_t status) {
	switch (status) {
	case undefined:
		return "undefined";
	case initialisation:
