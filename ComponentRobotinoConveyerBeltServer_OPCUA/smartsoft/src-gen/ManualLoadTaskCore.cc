...
int ManualLoadTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int ManualLoadTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int ManualLoadTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
