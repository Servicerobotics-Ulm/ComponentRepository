...
UnLoadTaskCore::UnLoadTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
UnLoadTaskCore::~UnLoadTaskCore()
{
}

int UnLoadTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int UnLoadTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int UnLoadTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void UnLoadTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const UnLoadTask* unLoadTask = dynamic_cast<const UnLoadTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(unLoadTask);
		}
	}
}

void UnLoadTaskCore::attach_interaction_observer(UnLoadTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void UnLoadTaskCore::detach_interaction_observer(UnLoadTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int UnLoadTaskCore::execute_protected_region()
{
...
void UnLoadTaskCore::updateAllCommObjects()
{
...
void UnLoadTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int UnLoadTaskCore::getPreviousCommObjId()
{
