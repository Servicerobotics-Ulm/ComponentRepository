...
SignalErrorTaskCore::SignalErrorTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
SignalErrorTaskCore::~SignalErrorTaskCore()
{
}

int SignalErrorTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int SignalErrorTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int SignalErrorTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void SignalErrorTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const SignalErrorTask* signalErrorTask = dynamic_cast<const SignalErrorTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(signalErrorTask);
		}
	}
}

void SignalErrorTaskCore::attach_interaction_observer(SignalErrorTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void SignalErrorTaskCore::detach_interaction_observer(SignalErrorTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int SignalErrorTaskCore::execute_protected_region()
{
...
void SignalErrorTaskCore::updateAllCommObjects()
{
...
void SignalErrorTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int SignalErrorTaskCore::getPreviousCommObjId()
{
