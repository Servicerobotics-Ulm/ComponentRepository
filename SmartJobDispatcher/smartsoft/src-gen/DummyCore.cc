...
Smart::StatusCode DummyCore::taskResultOutPut(CommBasicObjects::CommTaskMessage &taskResultOutDataObject)
{
	Smart::StatusCode result = COMP->taskResultOutWrapper->put(taskResultOutDataObject);
	if(useLogging == true) {
		//FIXME: use logging
...
// this method is meant to be used in derived classes
Smart::StatusCode DummyCore::jobEventServerPut(CommBasicObjects::CommTaskEventState &eventState)
{
	Smart::StatusCode result = COMP->jobEventServerWrapper->put(eventState);
	if(useLogging == true) {
		//FIXME: use logging
		//Smart::LOGGER->log(pushLoggingId+1, getCurrentUpdateCount(), getPreviousCommObjId());
	}
