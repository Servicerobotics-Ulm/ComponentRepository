...
DataTaskCore::DataTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
DataTaskCore::~DataTaskCore()
{
}

int DataTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int DataTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int DataTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void DataTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const DataTask* dataTask = dynamic_cast<const DataTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(dataTask);
		}
	}
}

void DataTaskCore::attach_interaction_observer(DataTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void DataTaskCore::detach_interaction_observer(DataTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int DataTaskCore::execute_protected_region()
{
...
void DataTaskCore::updateAllCommObjects()
{
...
void DataTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int DataTaskCore::getPreviousCommObjId()
{
