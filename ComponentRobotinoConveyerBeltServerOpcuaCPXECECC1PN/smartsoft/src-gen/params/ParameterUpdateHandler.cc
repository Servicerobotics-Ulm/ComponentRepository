...
SmartACE::CommParameterResponse ParamUpdateHandler::handleParametersNamed(const SmartACE::CommParameterRequest& request)
{
	...
	else if (tag == "SETSTATIONID")
	{
		...
		if(request.getInteger("id", temp_id) == 0) {
			commitState.SetStationID.id = temp_id;
		} else {
...
void ParamUpdateHandler::loadParameter(SmartACE::SmartIniParameter &parameter)
{
	...
	try
	{
		...
		if(parameter.getString("OPCUAstatic", "rootObjectPath", commitState.OPCUAstatic.rootObjectPath))
		{
			globalState.OPCUAstatic.rootObjectPath = commitState.OPCUAstatic.rootObjectPath;
		}
		...
		if(parameter.getInteger("SetStationID", "id", commitState.SetStationID.id))
		{
			globalState.SetStationID.id = commitState.SetStationID.id;
		}
