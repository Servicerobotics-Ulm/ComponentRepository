...
WebotsAPITaskCore::WebotsAPITaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
WebotsAPITaskCore::~WebotsAPITaskCore()
{
}

int WebotsAPITaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int WebotsAPITaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int WebotsAPITaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void WebotsAPITaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const WebotsAPITask* webotsAPITask = dynamic_cast<const WebotsAPITask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(webotsAPITask);
		}
	}
}

void WebotsAPITaskCore::attach_interaction_observer(WebotsAPITaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void WebotsAPITaskCore::detach_interaction_observer(WebotsAPITaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int WebotsAPITaskCore::execute_protected_region()
{
...
void WebotsAPITaskCore::updateAllCommObjects()
{
...
int WebotsAPITaskCore::getPreviousCommObjId()
{
