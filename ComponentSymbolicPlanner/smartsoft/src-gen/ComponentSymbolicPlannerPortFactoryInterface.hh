//--------------------------------------------------------------------------
// Code generated by the SmartSoft MDSD Toolchain
// The SmartSoft Toolchain has been developed by:
//  
// Service Robotics Research Center
// University of Applied Sciences Ulm
// Prittwitzstr. 10
// 89075 Ulm (Germany)
//
// Information about the SmartSoft MDSD Toolchain is available at:
// www.servicerobotik-ulm.de
//
// Please do not modify this file. It will be re-generated
// running the code generator.
//--------------------------------------------------------------------------

#ifndef COMPONENTSYMBOLICPLANNER_PORTFACTORYINTERFACE_HH_
#define COMPONENTSYMBOLICPLANNER_PORTFACTORYINTERFACE_HH_

// include communication objects
#include <DomainSymbolicPlanner/CommSymbolicPlannerPlan.hh>
#include <DomainSymbolicPlanner/CommSymbolicPlannerPlanACE.hh>
#include <DomainSymbolicPlanner/CommSymbolicPlannerRequest.hh>
#include <DomainSymbolicPlanner/CommSymbolicPlannerRequestACE.hh>

#include <chrono>

// include component's main class
#include "ComponentSymbolicPlanner.hh"

// forward declaration
class ComponentSymbolicPlanner;

class ComponentSymbolicPlannerPortFactoryInterface {
public:
	ComponentSymbolicPlannerPortFactoryInterface() { };
	virtual ~ComponentSymbolicPlannerPortFactoryInterface() { };

	virtual void initialize(ComponentSymbolicPlanner *component, int argc, char* argv[]) = 0;
	virtual int onStartup() = 0;

	
	virtual Smart::IQueryServerPattern<DomainSymbolicPlanner::CommSymbolicPlannerRequest, DomainSymbolicPlanner::CommSymbolicPlannerPlan,SmartACE::QueryId> * createSymbolicPlannerQueryServer(const std::string &serviceName) = 0;

	virtual int onShutdown(const std::chrono::steady_clock::duration &timeoutTime=std::chrono::seconds(2)) = 0;
	virtual void destroy() = 0;
};

#endif /* COMPONENTSYMBOLICPLANNER_PORTFACTORYINTERFACE_HH_ */
