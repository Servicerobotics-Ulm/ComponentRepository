...
class CoMPSSimpleManipulation:
    ...
    def sampleIKForPose(self, params):
        try:
            ...
            distToTarget = sqrt(params['targetLoc3D'][0]*params['targetLoc3D'][0]) + (params['targetLoc3D'][1]*params['targetLoc3D'][1]) + (params['targetLoc3D'][2]*params['targetLoc3D'][2])
            print("Distance to Target: ", distToTarget)
            print("TIMO BLENDER SAMPLE IK FOR POSE")
            ...
            print "-----------------------------------------------------"
            start = time.time()
            print ("INPUT TO COMPS: ", 'RunCBiRRT ' + objectString + ' filename cSampleIKtraj.txt psample 0.25 timelimit 3 %s'%TSRChainString1)
