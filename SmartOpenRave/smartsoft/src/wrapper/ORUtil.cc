		...
		SpecificManipulator::Manipulator& robot, const string& planner, double speedMulti, bool waitForAnimation)
{
	...
	PyTuple_SetItem(args, 5, createPyList(test1)); //maskWithGripper
	PyTuple_SetItem(args, 6, createPyList(test1)); //maskWithoutGripper
...
int ORUtil::planPathCOMPS(vector<TrajectoryPoint>& resultTraj)
{
	SmartACE::SmartGuard guard(this->mutex);

	int result;

	PyObject* pyResTraj = PyObject_CallMethod(this->pClass, (char*) "planPathCOMPS", ((char*) ""));
	if (pyResTraj != Py_None) {
		result = this->manipulator->parseTrajectoryTiagoArm(pyResTraj, resultTraj);

		for (int i = 0; i < resultTraj.size(); i++) {
			std::cout << "[" << i << "]:" << std::endl;
			std::cout << "jointValues" << std::endl;
			for (int j = 0; j < resultTraj[i].jointValues.size(); j++) {
				std::cout << resultTraj[i].jointValues[j] << ",";
			}
			std::cout << "[" << i << "]:" << std::endl;
			std::cout << "gripperJointValues" << std::endl;
			for (int j = 0; j < resultTraj[i].gripperJointValues.size(); j++) {
				std::cout << resultTraj[i].gripperJointValues[j] << ",";
			}
			std::cout << "[" << i << "]:" << std::endl;
			std::cout << "Time: " << resultTraj[i].time << std::endl;
			std::cout << endl;
		}
