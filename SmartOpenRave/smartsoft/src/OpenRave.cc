...
void OpenRave::syncManipulator()
{
	...
	if (COMP->getGlobalState().getPortParameter().getWithManipulator())
	{
		...
		do
		{
			//CHS::StatusCode status = COMP->manipulatorStateQueryClient->query(commVoid, manipulatorState);
		...
		if (!this->initial_sync)
		{
			...
			// 15.05.18 Timo: Prevents finding an ik solution with comps and tiago arm when executed
			// -> There seems to be no transformation necessary for the tiago arm
			if (specificManipulator->getOpenRaveName() != "TiagoArm") {
		...
		for (u_int32_t i = 0; i < manipulatorState.get_manipulator_state().get_joint_count(); ++i)
		{
			...
			std::cout << "joint [" << i << "]" << manipulatorState.get_manipulator_state().get_joint_angle(i) << std::endl;
		}
	...
	if (COMP->getGlobalState().getPortParameter().getWithGripper() && !this->obj_grasped)
	{
		...
		do
		{
			//CHS::StatusCode status = COMP->gripperStateQueryClient->query(commVoid, gripperState);
	...
	switch(OPENRAVE->getParameter().planningMode)
	{
		case OpenRave::OpenRaveParameter::PathPlanningMode::NORMAL:
		{
			if (specificManipulator->getOpenRaveName() != "TiagoArm") {
				...
				// Timo: This was added, because planning for tiago arm with openRave function does not work, but COMPS planner works
				result = COMP->OpenRaveWrapper.planPathCOMPS(trajectory);
			}
			break;
		}
...
void OpenRave::loadEnvironment(unsigned int envId)
{
	...
	//CHS::StatusCode status = COMP->environmentQueryClient->query(request, response);
...
//		}
		...
		//CommBasicObjects::CommPose3d sPose = object.getObjectSurfacePosesCopy()[0]; // This data is not available in normal object recognition
