...
TiagoArm::TiagoArm()
{
	...
	this->robotURI = "tiagoArm.robot.xml";
	this->manipulatorName = "arm";
	this->openRaveName = "TiagoArm";
	this->manipulatorType = "TiagoArm";
	...
	activeArmDofs.push_back(6);

	//activeGripperDofs.push_back(7);
...
bool TiagoArm::convertRealAnglesToOpenRaveAngles(const std::vector<double>& realAngles, std::vector<double>& openRaveAngles)
{
...
bool TiagoArm::convertOpenRaveAnglesToRealAngles(const std::vector<double>& openRaveAngles, std::vector<double>& realAngles)
{
//	if (realAngles.size() != openRaveAngles.size())
//	{
//		MessageHandler::handleMessage(
...
const vector<int>& TiagoArm::getActiveDofs()
{
 return this->activeDofs;
}

const vector<int>& TiagoArm::getActiveArmDofs()
{
	return this->activeArmDofs;
}

const vector<int>& TiagoArm::getActiveGripperDofs()
{
	return this->activeGripperDofs;
}

bool TiagoArm::removeDummyJointAngles(std::vector<double>& angles)
{
	//Model of UR6855A got no dummy joints
	return true;
}

arma::mat TiagoArm::getOffsetRealManipulatortoOpenRaveManipulator()
{
...
arma::mat TiagoArm::getSimulationTransform()
{
	arma::mat mat = arma::eye<arma::mat>(4, 4);
...
void TiagoArm::getSimulationJointAngles(std::vector<double>& angles)
{
...
void TiagoArm::parseIterParameters(ORUtil::PlanningParameter& params, ORUtil& orutil)
{
	//params.azimuth += 90;
	//params.elevation += 90;
}
...
double TiagoArm::wrapToPi(double a)
{
	return wrapTo2Pi(a + M_PI) - M_PI;
}

double TiagoArm::wrapTo2Pi(double a)
{
	bool was_neg = a < 0;
	a = fmod(a, 2 * M_PI);
