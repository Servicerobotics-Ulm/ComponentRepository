...
SpeechTaskCore::SpeechTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
SpeechTaskCore::~SpeechTaskCore()
{
}

int SpeechTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int SpeechTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int SpeechTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void SpeechTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const SpeechTask* speechTask = dynamic_cast<const SpeechTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(speechTask);
		}
	}
}

void SpeechTaskCore::attach_interaction_observer(SpeechTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void SpeechTaskCore::detach_interaction_observer(SpeechTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int SpeechTaskCore::execute_protected_region()
{
...
void SpeechTaskCore::updateAllCommObjects()
{
...
void SpeechTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int SpeechTaskCore::getPreviousCommObjId()
{
