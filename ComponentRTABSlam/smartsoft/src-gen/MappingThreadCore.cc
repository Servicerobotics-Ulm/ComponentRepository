...
MappingThreadCore::MappingThreadCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
MappingThreadCore::~MappingThreadCore()
{
}


void MappingThreadCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const MappingThread* mappingThread = dynamic_cast<const MappingThread*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(mappingThread);
		}
	}
}

void MappingThreadCore::attach_interaction_observer(MappingThreadObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void MappingThreadCore::detach_interaction_observer(MappingThreadObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int MappingThreadCore::execute_protected_region()
{
...
void MappingThreadCore::updateAllCommObjects()
{
...
void MappingThreadCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int MappingThreadCore::getPreviousCommObjId()
{
