...
LocalizationThreadCore::LocalizationThreadCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
LocalizationThreadCore::~LocalizationThreadCore()
{
}


void LocalizationThreadCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const LocalizationThread* localizationThread = dynamic_cast<const LocalizationThread*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(localizationThread);
		}
	}
}

void LocalizationThreadCore::attach_interaction_observer(LocalizationThreadObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void LocalizationThreadCore::detach_interaction_observer(LocalizationThreadObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int LocalizationThreadCore::execute_protected_region()
{
...
void LocalizationThreadCore::updateAllCommObjects()
{
...
void LocalizationThreadCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int LocalizationThreadCore::getPreviousCommObjId()
{
