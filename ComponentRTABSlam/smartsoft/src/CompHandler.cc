...
void CompHandler::onStartup() 
{
	...
	bool IsLocalization = COMP->getGlobalState().getTaskSelection().getIsLocalization();

	if(IsLocalization)
	{
	status = COMP->connectRgbd_client(COMP->connections.rgbd_client.serverName, COMP->connections.rgbd_client.serviceName);

    COMP->localizationThread->start();
	}else
	{

    status = COMP->connectRgbd_client(COMP->connections.rgbd_client.serverName, COMP->connections.rgbd_client.serviceName);
        COMP->mappingThread->start();
	}
