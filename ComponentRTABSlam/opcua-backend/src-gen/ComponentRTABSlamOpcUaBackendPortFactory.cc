...
#include "ComponentRTABSlamOpcUaBackendPortFactory.hh"
...
static ComponentRTABSlamOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentRTABSlamOpcUaBackendPortFactory::ComponentRTABSlamOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentRTABSlam::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentRTABSlamOpcUaBackendPortFactory::~ComponentRTABSlamOpcUaBackendPortFactory()
{  }

void ComponentRTABSlamOpcUaBackendPortFactory::initialize(ComponentRTABSlam *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentRTABSlamOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentRTABSlamOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<CommBasicObjects::CommBaseState> * ComponentRTABSlamOpcUaBackendPortFactory::createBaseStatePushNewestClient()
{
	return new SeRoNet::OPCUA::Client::PushClient<CommBasicObjects::CommBaseState>(componentImpl);
}

Smart::IPushClientPattern<DomainVision::CommRGBDImage> * ComponentRTABSlamOpcUaBackendPortFactory::createRgbd_client()
{
...
int ComponentRTABSlamOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentRTABSlamOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentRTABSlamOpcUaBackendPortFactory::destroy()
{
