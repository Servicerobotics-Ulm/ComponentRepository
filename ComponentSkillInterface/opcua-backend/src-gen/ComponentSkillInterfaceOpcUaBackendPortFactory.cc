...
#include "ComponentSkillInterfaceOpcUaBackendPortFactory.hh"
...
static ComponentSkillInterfaceOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentSkillInterfaceOpcUaBackendPortFactory::ComponentSkillInterfaceOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentSkillInterface::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentSkillInterfaceOpcUaBackendPortFactory::~ComponentSkillInterfaceOpcUaBackendPortFactory()
{  }

void ComponentSkillInterfaceOpcUaBackendPortFactory::initialize(ComponentSkillInterface *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentSkillInterfaceOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentSkillInterfaceOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IQueryClientPattern<CommBasicObjects::CommKBRequest, CommBasicObjects::CommKBResponse> * ComponentSkillInterfaceOpcUaBackendPortFactory::createKBQueryClient()
{
...
int ComponentSkillInterfaceOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentSkillInterfaceOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentSkillInterfaceOpcUaBackendPortFactory::destroy()
{
