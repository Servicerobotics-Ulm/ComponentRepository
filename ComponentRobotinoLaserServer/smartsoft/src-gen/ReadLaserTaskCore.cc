...
ReadLaserTaskCore::ReadLaserTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
ReadLaserTaskCore::~ReadLaserTaskCore()
{
}

int ReadLaserTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int ReadLaserTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int ReadLaserTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void ReadLaserTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const ReadLaserTask* readLaserTask = dynamic_cast<const ReadLaserTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(readLaserTask);
		}
	}
}

void ReadLaserTaskCore::attach_interaction_observer(ReadLaserTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void ReadLaserTaskCore::detach_interaction_observer(ReadLaserTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int ReadLaserTaskCore::execute_protected_region()
{
...
void ReadLaserTaskCore::updateAllCommObjects()
{
...
void ReadLaserTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int ReadLaserTaskCore::getPreviousCommObjId()
{
