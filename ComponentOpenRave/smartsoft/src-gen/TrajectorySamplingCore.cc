...
TrajectorySamplingCore::TrajectorySamplingCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
TrajectorySamplingCore::~TrajectorySamplingCore()
{
}

int TrajectorySamplingCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int TrajectorySamplingCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int TrajectorySamplingCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void TrajectorySamplingCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const TrajectorySampling* trajectorySampling = dynamic_cast<const TrajectorySampling*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(trajectorySampling);
		}
	}
}

void TrajectorySamplingCore::attach_interaction_observer(TrajectorySamplingObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void TrajectorySamplingCore::detach_interaction_observer(TrajectorySamplingObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int TrajectorySamplingCore::execute_protected_region()
{
...
void TrajectorySamplingCore::updateAllCommObjects()
{
...
void TrajectorySamplingCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int TrajectorySamplingCore::getPreviousCommObjId()
{
