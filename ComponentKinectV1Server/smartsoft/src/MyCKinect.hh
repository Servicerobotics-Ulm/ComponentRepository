/* +---------------------------------------------------------------------------+
   ...
   |                                                                           |
   +---------------------------------------------------------------------------+ */
#ifndef mrpt_MYCKinect_H
#define mrpt_MYCKinect_H
...
namespace mrpt
{
	namespace hwdrivers
	{
		/** A class for grabing "range images", intensity images (either RGB or IR) and other information from an Xbox Kinect sensor.
		  ...
		  */
		class HWDRIVERS_IMPEXP  MyCKinect : public mrpt::hwdrivers::CGenericSensor
		{
			DEFINE_GENERIC_SENSOR(MyCKinect)
		...
		public:
			...
			/** Try to open the camera (set all the parameters before calling this) - users may also call initialize(), which in turn calls this method.
			  ...
			  */
			void open(int rgb_mode = 1, int depth_mode = 1);

			/**
			 * After initialization start the video and depth streams
			 */
			...
			const freenect_frame_mode get_desired_rgb_frame_Mode(int rgb_mode);
			const freenect_frame_mode get_desired_depth_frame_Mode(int depth_mode);
	...
	{
		template <>
		struct TEnumTypeFiller<hwdrivers::MyCKinect::TVideoChannel>
		{
			typedef hwdrivers::MyCKinect::TVideoChannel enum_t;
			static void fill(bimap<enum_t,std::string>  &m_map)
			{
				m_map.insert(hwdrivers::MyCKinect::VIDEO_CHANNEL_RGB, "VIDEO_CHANNEL_RGB");
				m_map.insert(hwdrivers::MyCKinect::VIDEO_CHANNEL_IR,  "VIDEO_CHANNEL_IR");
			}
		};
	} // End of namespace
