...
void ColorImageQueryHandler::handleQuery(const QueryId &id, const CommBasicObjects::CommVoid& request)
{
		...
		if(status == Smart::SMART_OK)
		{
			...
			if (COMP->stateSlave->tryAcquire("pushimage")==Smart::SMART_OK)
			{
				...
				if(COMP->newestImage != NULL){
					answer.set_parameters(COMP->newestImage->getColor_image().get_width(),COMP->newestImage->getColor_image().get_height(), DomainVision::FormatType::RGB24);
					answer.set_data(COMP->newestImage->getColor_image().get_data());
					answer.set_sequence_counter(COMP->newestImage->getSeq_count());
				}
	...
	//			zero_velocity.set_omega_turret(0);
				...
				DomainVision::CommRGBDImage answer_kinect;

				{
				// get newest image from Kinect
				SmartACE::SmartGuard guard(COMP->kinectMutex);
				COMP->device->get_rgbd_image(answer_kinect);
				guard.release();
				}

				answer_kinect.getDepth_image().setMin_distcance(COMP->getGlobalState().getHardware_properties().getMin_distance());
				answer_kinect.getDepth_image().setMax_distcance(COMP->getGlobalState().getHardware_properties().getMax_distance());
						...
						if (COMP->getGlobalState().getBase().getOn_ptu()) {
							...
							std::cout << "PTU POS: " << devicePoseState.get_base_state() << std::endl;
						...
						answer_kinect.setSensor_pose(sensorPose);
						answer_kinect.setBase_state(base_state);

						answer.set_parameters(COMP->newestImage->getColor_image().get_width(),COMP->newestImage->getColor_image().get_height(), DomainVision::FormatType::RGB24);
						answer.set_data(answer_kinect.getColor_image().get_data());
						answer.setSeq_count(answer_kinect.getSeq_count());
