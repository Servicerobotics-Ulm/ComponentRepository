...
#include "SmartRobotConsoleOpcUaBackendPortFactory.hh"
...
static SmartRobotConsoleOpcUaBackendPortFactory OpcUaBackendPortFactory;

SmartRobotConsoleOpcUaBackendPortFactory::SmartRobotConsoleOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	SmartRobotConsole::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

SmartRobotConsoleOpcUaBackendPortFactory::~SmartRobotConsoleOpcUaBackendPortFactory()
{  }

void SmartRobotConsoleOpcUaBackendPortFactory::initialize(SmartRobotConsole *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int SmartRobotConsoleOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&SmartRobotConsoleOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
int SmartRobotConsoleOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int SmartRobotConsoleOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void SmartRobotConsoleOpcUaBackendPortFactory::destroy()
{
