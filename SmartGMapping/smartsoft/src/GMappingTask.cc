...
{
	std::cout << "constructor GMappingTask\n";
	processor = NULL;
	mapID=0;
}
...
processorLock.acquire();
	...
	do
	{
		...
		if(status != Smart::SMART_OK){
			std::cout<<"GMappingTask::initGMappingProcessor:"<<status<<" ERROR getting laser scan ... retry."<<std::endl;
			sleep(1);
		}
...
int GMappingTask::on_execute()
{
	...
	Smart::StatusCode status;
		...
		processorLock.acquire();
		if(Smart::SMART_OK == this->laserServiceInGetUpdate(laserscan))
		{
			const GMapping::RangeSensor * rs= m_frontLaser;
			GMapping::RangeReading reading(rs, 0);
		...
		//			std::cout<<"Start angle: "<<COMP->pi_to_pi(laserscan.get_scan_start_angle())<<"INDEX: "<<index<< "angle: "<<angle<<"res: "<<laserscan.get_scan_resolution()<<std::endl;
			...
			upd_pos.set_x (pose.x, 1);
				...
				this->localizationUpdateServiceOutPut(upd);
		...
		{
			std::cout<<"Error getting laser scan: "<<status<<std::endl;
		}
...
int GMappingTask::sendMap(){
	...
	this->currGridMapPushServiceOutPut( gm );
}
