...
#include "ComponentTCLSequencerOpcUaBackendPortFactory.hh"
...
static ComponentTCLSequencerOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentTCLSequencerOpcUaBackendPortFactory::ComponentTCLSequencerOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentTCLSequencer::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentTCLSequencerOpcUaBackendPortFactory::~ComponentTCLSequencerOpcUaBackendPortFactory()
{  }

void ComponentTCLSequencerOpcUaBackendPortFactory::initialize(ComponentTCLSequencer *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentTCLSequencerOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentTCLSequencerOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
int ComponentTCLSequencerOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentTCLSequencerOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentTCLSequencerOpcUaBackendPortFactory::destroy()
{
