...
int DevicePoseStateTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int DevicePoseStateTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int DevicePoseStateTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
