...
PtuTaskCore::PtuTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
PtuTaskCore::~PtuTaskCore()
{
}

int PtuTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int PtuTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int PtuTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void PtuTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const PtuTask* ptuTask = dynamic_cast<const PtuTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(ptuTask);
		}
	}
}

void PtuTaskCore::attach_interaction_observer(PtuTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void PtuTaskCore::detach_interaction_observer(PtuTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int PtuTaskCore::execute_protected_region()
{
...
void PtuTaskCore::updateAllCommObjects()
{
...
Smart::StatusCode PtuTaskCore::goalEventServerPut(DomainPTU::PTUGoalEventState &eventState)
{
...
void PtuTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int PtuTaskCore::getPreviousCommObjId()
{
