...
KeyboardInputTaskCore::KeyboardInputTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
KeyboardInputTaskCore::~KeyboardInputTaskCore()
{
}

int KeyboardInputTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int KeyboardInputTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int KeyboardInputTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void KeyboardInputTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const KeyboardInputTask* keyboardInputTask = dynamic_cast<const KeyboardInputTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(keyboardInputTask);
		}
	}
}

void KeyboardInputTaskCore::attach_interaction_observer(KeyboardInputTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void KeyboardInputTaskCore::detach_interaction_observer(KeyboardInputTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int KeyboardInputTaskCore::execute_protected_region()
{
...
void KeyboardInputTaskCore::updateAllCommObjects()
{
...
void KeyboardInputTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int KeyboardInputTaskCore::getPreviousCommObjId()
{
