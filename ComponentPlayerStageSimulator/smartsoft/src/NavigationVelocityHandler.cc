...
void NavigationVelocityHandler::on_NavigationVelocityServiceIn(const CommBasicObjects::CommNavigationVelocity &r)
{
    // read the received CommNavigationVelocity object
    const double v = r.get_vX();
    const double omega = r.get_omega();
    COMP->PlayerClientMutex.acquire();

    std::cout << "NavigationVelocity = " << r << std::endl;
    COMP->position_2d_proxy->SetSpeed(v/1000.0, omega);
