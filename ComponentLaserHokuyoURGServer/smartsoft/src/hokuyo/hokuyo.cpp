...
hokuyo::Laser::laserReadline(char *buf, int len, int timeout)
{
  ...
  while (current < len - 1)
  {
	...
	fd_set input;
	FD_ZERO(&input);
	FD_SET(laser_fd_, &input);
	struct timeval tv;
	tv.tv_sec  = 1;
	tv.tv_usec = 0;
	int n = select(laser_fd_+1, &input,  NULL, NULL, &tv );
	/* See if there was an error */
	if (n < 0)
	{
		HOKUYO_EXCEPT(hokuyo::Exception, "select failed" );
	}
	else if (n == 0)
	{
		HOKUYO_EXCEPT(hokuyo::Exception, "select TIMEOUT" );
	}
	else
	{
