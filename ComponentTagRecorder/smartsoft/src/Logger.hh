...
private:
	...
	std::string filename;
	std::map<unsigned int, std::vector<CommBasicObjects::CommPose3d>> data;
	std::map<unsigned int, CommBasicObjects::CommPose3d> data_average;
public:
	Logger();
	virtual ~Logger();
	void write_markerlist(const CommTrackingObjects::CommDetectedMarkerList& dml);
	std::map<unsigned int, CommBasicObjects::CommPose3d>& calculate_mean_poses();
