...
SmartACE::CommParameterResponse ParamUpdateHandler::handleParametersNamed(const SmartACE::CommParameterRequest& request)
{
	...
	else if (tag == "COMMTRACKINGOBJECTS.MARKERS.SAVE_TAGS_AS_JSON")
	{
		...
		if(answer.getResponse() == SmartACE::ParamResponseType::OK) {
			triggerHandler.handleCommTrackingObjects_Markers_SAVE_TAGS_AS_JSONCore(
			temp_dirname
			);
		}
	}
...
void ParamUpdateHandler::loadParameter(SmartACE::SmartIniParameter &parameter)
{
	...
	try
	{
		...
		//
		// parameter options
		if(parameter.getString("options", "log_path", commitState.options.log_path))
		{
			globalState.options.log_path = commitState.options.log_path;
		}
