...
void ParamUpdateHandler::loadParameter(SmartACE::SmartIniParameter &parameter)
{
	...
	try
	{
		...
		//
		// parameter options
		if(parameter.getBoolean("options", "debug", commitState.options.debug))
		{
			globalState.options.debug = commitState.options.debug;
		}
		if(parameter.getDouble("options", "decimate", commitState.options.decimate))
		{
			globalState.options.decimate = commitState.options.decimate;
		}
		if(parameter.getDouble("options", "decision_threshold", commitState.options.decision_threshold))
		{
			globalState.options.decision_threshold = commitState.options.decision_threshold;
		}
		if(parameter.getInteger("options", "numThreads", commitState.options.numThreads))
		{
			globalState.options.numThreads = commitState.options.numThreads;
		}
		if(parameter.getBoolean("options", "refine_edges", commitState.options.refine_edges))
		{
			globalState.options.refine_edges = commitState.options.refine_edges;
		}
		if(parameter.getDouble("options", "sigma", commitState.options.sigma))
		{
			globalState.options.sigma = commitState.options.sigma;
		}
		if(parameter.getString("options", "tag_family", commitState.options.tag_family))
		{
			globalState.options.tag_family = commitState.options.tag_family;
		}
		if(parameter.getDouble("options", "tag_size", commitState.options.tag_size))
		{
			globalState.options.tag_size = commitState.options.tag_size;
		}
