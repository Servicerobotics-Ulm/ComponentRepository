...
#include "ComponentWebots2DLidarOpcUaBackendPortFactory.hh"
...
static ComponentWebots2DLidarOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentWebots2DLidarOpcUaBackendPortFactory::ComponentWebots2DLidarOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentWebots2DLidar::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentWebots2DLidarOpcUaBackendPortFactory::~ComponentWebots2DLidarOpcUaBackendPortFactory()
{  }

void ComponentWebots2DLidarOpcUaBackendPortFactory::initialize(ComponentWebots2DLidar *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentWebots2DLidarOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentWebots2DLidarOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<CommBasicObjects::CommBaseState> * ComponentWebots2DLidarOpcUaBackendPortFactory::createBaseStateServiceIn()
{
	return new SeRoNet::OPCUA::Client::PushClient<CommBasicObjects::CommBaseState>(componentImpl);
}


Smart::IQueryServerPattern<CommBasicObjects::CommVoid, CommBasicObjects::CommMobileLaserScan> * ComponentWebots2DLidarOpcUaBackendPortFactory::createLaserQueryServiceAnsw(const std::string &serviceName)
{
...
Smart::IPushServerPattern<CommBasicObjects::CommMobileLaserScan> * ComponentWebots2DLidarOpcUaBackendPortFactory::createLaserServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<CommBasicObjects::CommMobileLaserScan>(componentImpl, serviceName);
}


int ComponentWebots2DLidarOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentWebots2DLidarOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentWebots2DLidarOpcUaBackendPortFactory::destroy()
{
