...
Laser1TaskCore::Laser1TaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
void Laser1TaskCore::notify_all_interaction_observers() {
	...
	if(const Laser1Task* laser1Task = dynamic_cast<const Laser1Task*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(laser1Task);
		}
	}
}

void Laser1TaskCore::attach_interaction_observer(Laser1TaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void Laser1TaskCore::detach_interaction_observer(Laser1TaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int Laser1TaskCore::execute_protected_region()
{
...
void Laser1TaskCore::updateAllCommObjects()
{
...
void Laser1TaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int Laser1TaskCore::getPreviousCommObjId()
{
