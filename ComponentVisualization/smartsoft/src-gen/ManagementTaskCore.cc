...
ManagementTaskCore::ManagementTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
ManagementTaskCore::~ManagementTaskCore()
{
}

int ManagementTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int ManagementTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int ManagementTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void ManagementTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const ManagementTask* managementTask = dynamic_cast<const ManagementTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(managementTask);
		}
	}
}

void ManagementTaskCore::attach_interaction_observer(ManagementTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void ManagementTaskCore::detach_interaction_observer(ManagementTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int ManagementTaskCore::execute_protected_region()
{
...
void ManagementTaskCore::updateAllCommObjects()
{
...
void ManagementTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int ManagementTaskCore::getPreviousCommObjId()
{
