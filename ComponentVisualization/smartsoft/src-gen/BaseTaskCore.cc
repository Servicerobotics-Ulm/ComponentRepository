...
BaseTaskCore::BaseTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
BaseTaskCore::~BaseTaskCore()
{
}

int BaseTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int BaseTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int BaseTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void BaseTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const BaseTask* baseTask = dynamic_cast<const BaseTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(baseTask);
		}
	}
}

void BaseTaskCore::attach_interaction_observer(BaseTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void BaseTaskCore::detach_interaction_observer(BaseTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int BaseTaskCore::execute_protected_region()
{
...
void BaseTaskCore::updateAllCommObjects()
{
...
void BaseTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int BaseTaskCore::getPreviousCommObjId()
{
