...
Laser3TaskCore::Laser3TaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
Laser3TaskCore::~Laser3TaskCore()
{
}

int Laser3TaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int Laser3TaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int Laser3TaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void Laser3TaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const Laser3Task* laser3Task = dynamic_cast<const Laser3Task*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(laser3Task);
		}
	}
}

void Laser3TaskCore::attach_interaction_observer(Laser3TaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void Laser3TaskCore::detach_interaction_observer(Laser3TaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int Laser3TaskCore::execute_protected_region()
{
...
void Laser3TaskCore::updateAllCommObjects()
{
...
void Laser3TaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int Laser3TaskCore::getPreviousCommObjId()
{
