...
DepthTaskCore::DepthTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
DepthTaskCore::~DepthTaskCore()
{
}

int DepthTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int DepthTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int DepthTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void DepthTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const DepthTask* depthTask = dynamic_cast<const DepthTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(depthTask);
		}
	}
}

void DepthTaskCore::attach_interaction_observer(DepthTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void DepthTaskCore::detach_interaction_observer(DepthTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int DepthTaskCore::execute_protected_region()
{
...
void DepthTaskCore::updateAllCommObjects()
{
...
void DepthTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int DepthTaskCore::getPreviousCommObjId()
{
