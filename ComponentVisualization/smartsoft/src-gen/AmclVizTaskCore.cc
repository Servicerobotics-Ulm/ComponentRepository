...
AmclVizTaskCore::AmclVizTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
AmclVizTaskCore::~AmclVizTaskCore()
{
}

int AmclVizTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int AmclVizTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int AmclVizTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void AmclVizTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const AmclVizTask* amclVizTask = dynamic_cast<const AmclVizTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(amclVizTask);
		}
	}
}

void AmclVizTaskCore::attach_interaction_observer(AmclVizTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void AmclVizTaskCore::detach_interaction_observer(AmclVizTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int AmclVizTaskCore::execute_protected_region()
{
...
void AmclVizTaskCore::updateAllCommObjects()
{
...
void AmclVizTaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int AmclVizTaskCore::getPreviousCommObjId()
{
