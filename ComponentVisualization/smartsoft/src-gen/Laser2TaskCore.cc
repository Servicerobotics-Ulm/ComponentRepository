...
Laser2TaskCore::Laser2TaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
Laser2TaskCore::~Laser2TaskCore()
{
}

int Laser2TaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int Laser2TaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int Laser2TaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void Laser2TaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const Laser2Task* laser2Task = dynamic_cast<const Laser2Task*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(laser2Task);
		}
	}
}

void Laser2TaskCore::attach_interaction_observer(Laser2TaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void Laser2TaskCore::detach_interaction_observer(Laser2TaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int Laser2TaskCore::execute_protected_region()
{
...
void Laser2TaskCore::updateAllCommObjects()
{
...
void Laser2TaskCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int Laser2TaskCore::getPreviousCommObjId()
{
