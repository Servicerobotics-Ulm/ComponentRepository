...
#include "ComponentRosDockOpcUaBackendPortFactory.hh"
...
ComponentRosDockOpcUaBackendPortFactory::ComponentRosDockOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentRosDock::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentRosDockOpcUaBackendPortFactory::~ComponentRosDockOpcUaBackendPortFactory()
{  }

void ComponentRosDockOpcUaBackendPortFactory::initialize(ComponentRosDock *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentRosDockOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentRosDockOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
int ComponentRosDockOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
