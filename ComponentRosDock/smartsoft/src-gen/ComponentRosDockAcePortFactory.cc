...
static ComponentRosDockAcePortFactory acePortFactory;

ComponentRosDockAcePortFactory::ComponentRosDockAcePortFactory()
{  
	componentImpl = 0;
	ComponentRosDock::instance()->addPortFactory("ACE_SmartSoft", this);
}

ComponentRosDockAcePortFactory::~ComponentRosDockAcePortFactory()
{  }

void ComponentRosDockAcePortFactory::initialize(ComponentRosDock *component, int argc, char* argv[])
{
	if(component->connections.component.defaultScheduler != "DEFAULT") {
		...
		componentImpl = new ComponentRosDockImpl(component->connections.component.name, argc, argv, sched_params);
	} else {
		// create new instance of the SmartSoft component
		componentImpl = new ComponentRosDockImpl(component->connections.component.name, argc, argv);
	}
}

int ComponentRosDockAcePortFactory::onStartup()
{
...
int ComponentRosDockAcePortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
	componentImpl->stopComponentInfrastructure(timeoutTime);
	return 0;
}

void ComponentRosDockAcePortFactory::destroy()
{
