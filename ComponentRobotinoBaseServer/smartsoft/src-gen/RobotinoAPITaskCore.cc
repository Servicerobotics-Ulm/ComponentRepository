...
RobotinoAPITaskCore::RobotinoAPITaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
RobotinoAPITaskCore::~RobotinoAPITaskCore()
{
}

int RobotinoAPITaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int RobotinoAPITaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int RobotinoAPITaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void RobotinoAPITaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const RobotinoAPITask* robotinoAPITask = dynamic_cast<const RobotinoAPITask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(robotinoAPITask);
		}
	}
}

void RobotinoAPITaskCore::attach_interaction_observer(RobotinoAPITaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void RobotinoAPITaskCore::detach_interaction_observer(RobotinoAPITaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int RobotinoAPITaskCore::execute_protected_region()
{
...
void RobotinoAPITaskCore::updateAllCommObjects()
{
...
int RobotinoAPITaskCore::getPreviousCommObjId()
{
