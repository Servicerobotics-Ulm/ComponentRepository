...
SignalStateTaskCore::SignalStateTaskCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
SignalStateTaskCore::~SignalStateTaskCore()
{
}

int SignalStateTaskCore::start() {
	this->resetTrigger();
	return SmartACE::Task::start();
}

int SignalStateTaskCore::start(const ACE_Sched_Params &sched_params, const int &cpuAffinity) {
	return SmartACE::Task::start(sched_params, cpuAffinity);
}

int SignalStateTaskCore::stop(const bool wait_till_stopped) {
	this->cancelTrigger();
	return SmartACE::Task::stop(wait_till_stopped);
}


void SignalStateTaskCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const SignalStateTask* signalStateTask = dynamic_cast<const SignalStateTask*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(signalStateTask);
		}
	}
}

void SignalStateTaskCore::attach_interaction_observer(SignalStateTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void SignalStateTaskCore::detach_interaction_observer(SignalStateTaskObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int SignalStateTaskCore::execute_protected_region()
{
...
void SignalStateTaskCore::updateAllCommObjects()
{
...
int SignalStateTaskCore::getPreviousCommObjId()
{
